CREATE OR REPLACE VIEW public.d_23 AS 
SELECT __dummy__.COL0 AS ID,__dummy__.COL1 AS VALUE 
FROM (SELECT d_23_a2_0.COL0 AS COL0, d_23_a2_0.COL1 AS COL1 
...
temprecΔ_ins_s public.s%ROWTYPE;
  BEGIN
    -- IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'd_23_delta_action_flag') THEN
        -- RAISE LOG 'execute procedure d_23_delta_action';
        CREATE TEMPORARY TABLE IF NOT EXISTS d_23_delta_action_flag ON COMMIT DROP AS (SELECT true as finish);
...
FROM __temp__Δ_ins_d_23 AS __temp__Δ_ins_d_23_a2_0  ) AS new_d_23_a2 
WHERE new_d_23_a2.COL1 = s_a2_0.VALUE AND new_d_23_a2.COL0 = s_a2_0.ID ) ) AS Δ_del_s_a2_0  ) AS Δ_del_s_extra_alias;
...
FROM (SELECT new_d_23_a2_0.COL0 AS COL0, new_d_23_a2_0.COL1 AS COL1 
FROM (SELECT d_23_a2_0.ID AS COL0, d_23_a2_0.VALUE AS COL1 
FROM public.d_23 AS d_23_a2_0 
WHERE NOT EXISTS ( SELECT * 
FROM __temp__Δ_del_d_23 AS __temp__Δ_del_d_23_a2 
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
        GET STACKED DIAGNOSTICS text_var1 = RETURNED_SQLSTATE,
                                text_var2 = PG_EXCEPTION_DETAIL,
                                text_var3 = MESSAGE_TEXT;
        RAISE SQLSTATE 'DA000' USING MESSAGE = 'error on the trigger of public.d_23 ; error code: ' || text_var1 || ' ; ' || text_var2 ||' ; ' || text_var3;
        RETURN NULL;
  END;
$$;

CREATE OR REPLACE FUNCTION public.d_23_materialization()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  ...
  text_var3 text;
  BEGIN
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = '__temp__Δ_ins_d_23' OR table_name = '__temp__Δ_del_d_23')
    THEN
        -- RAISE LOG 'execute procedure d_23_materialization';
        ...
        CREATE CONSTRAINT TRIGGER __temp__d_23_trigger_delta_action
        AFTER INSERT OR UPDATE OR DELETE ON 
            __temp__Δ_del_d_23 DEFERRABLE INITIALLY IMMEDIATE 
            FOR EACH ROW EXECUTE PROCEDURE public.d_23_delta_action();
    END IF;
    RETURN NULL;
  EXCEPTION
...
DROP TRIGGER IF EXISTS d_23_trigger_materialization ON public.d_23;
CREATE TRIGGER d_23_trigger_materialization
    BEFORE INSERT OR UPDATE OR DELETE ON
      public.d_23 FOR EACH STATEMENT EXECUTE PROCEDURE public.d_23_materialization();

CREATE OR REPLACE FUNCTION public.d_23_update()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  ...
  text_var3 text;
  BEGIN
    -- RAISE LOG 'execute procedure d_23_update';
    IF TG_OP = 'INSERT' THEN
      -- RAISE LOG 'NEW: %', NEW;
      IF (SELECT count(*) FILTER (WHERE j.value = jsonb 'null') FROM  jsonb_each(to_jsonb(NEW)) j) > 0 THEN 
        RAISE check_violation USING MESSAGE = 'Invalid update on view: view does not accept null value';
      END IF;
      DELETE FROM __temp__Δ_del_d_23 WHERE ROW(ID,VALUE) = NEW;
      INSERT INTO __temp__Δ_ins_d_23 SELECT (NEW).*; 
    ELSIF TG_OP = 'UPDATE' THEN
      IF (SELECT count(*) FILTER (WHERE j.value = jsonb 'null') FROM  jsonb_each(to_jsonb(NEW)) j) > 0 THEN 
        RAISE check_violation USING MESSAGE = 'Invalid update on view: view does not accept null value';
      END IF;
      DELETE FROM __temp__Δ_ins_d_23 WHERE ROW(ID,VALUE) = OLD;
      INSERT INTO __temp__Δ_del_d_23 SELECT (OLD).*;
      DELETE FROM __temp__Δ_del_d_23 WHERE ROW(ID,VALUE) = NEW;
      INSERT INTO __temp__Δ_ins_d_23 SELECT (NEW).*; 
...
DROP TRIGGER IF EXISTS d_23_trigger_update ON public.d_23;
CREATE TRIGGER d_23_trigger_update
    INSTEAD OF INSERT OR UPDATE OR DELETE ON
      public.d_23 FOR EACH ROW EXECUTE PROCEDURE public.d_23_update();
