...
DROP MATERIALIZED VIEW IF EXISTS public.__dummy__materialized_d_34;

CREATE  MATERIALIZED VIEW public.__dummy__materialized_d_34 AS 
SELECT * FROM public.d_34;
...
echo "true"
$$ LANGUAGE plsh;
CREATE OR REPLACE FUNCTION public.d_34_detect_update()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
  ...
  IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'd_34_delta_action_flag') THEN
    insertion_data := (SELECT (array_to_json(array_agg(t)))::text FROM (SELECT * FROM public.d_34 EXCEPT SELECT * FROM public.__dummy__materialized_d_34) as t);
    IF insertion_data IS NOT DISTINCT FROM NULL THEN 
        insertion_data := '[]';
    END IF; 
    deletion_data := (SELECT (array_to_json(array_agg(t)))::text FROM (SELECT * FROM public.__dummy__materialized_d_34 EXCEPT SELECT * FROM public.d_34) as t);
    ...
    IF (insertion_data IS DISTINCT FROM '[]') OR (deletion_data IS DISTINCT FROM '[]') THEN 
        ...
        IF NOT (user_name = 'dejima') THEN 
            ...
            IF result = 'true' THEN 
                REFRESH MATERIALIZED VIEW public.__dummy__materialized_d_34;
                FOR func IN (select distinct trigger_schema||'.non_trigger_'||substring(action_statement, 19) as function 
                from information_schema.triggers where trigger_schema = 'public' and event_object_table='d_34'
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        RAISE SQLSTATE 'DA000' USING MESSAGE = 'error on the function (non_trigger_)public.d_34_detect_update() ; error code: ' || text_var1 || ' ; ' || text_var2 ||' ; ' || text_var3;
        RETURN NULL;
  END;
$$;
...
DROP TRIGGER IF EXISTS s_detect_update_d_34 ON public.s;
        CREATE TRIGGER s_detect_update_d_34
            AFTER INSERT OR UPDATE OR DELETE ON
            public.s FOR EACH STATEMENT EXECUTE PROCEDURE public.d_34_detect_update();
...
DROP TABLE Δ_ins_s;
        ...
        insertion_data := (SELECT (array_to_json(array_agg(t)))::text FROM (SELECT * FROM __temp__Δ_ins_d_34 EXCEPT SELECT * FROM public.__dummy__materialized_d_34) as t);
        IF insertion_data IS NOT DISTINCT FROM NULL THEN 
            insertion_data := '[]';
        END IF; 
        deletion_data := (SELECT (array_to_json(array_agg(t)))::text FROM (SELECT * FROM __temp__Δ_del_d_34 INTERSECT SELECT * FROM public.__dummy__materialized_d_34) as t);
        IF deletion_data IS NOT DISTINCT FROM NULL THEN 
            deletion_data := '[]';
        END IF; 
        IF (insertion_data IS DISTINCT FROM '[]') OR (deletion_data IS DISTINCT FROM '[]') THEN 
