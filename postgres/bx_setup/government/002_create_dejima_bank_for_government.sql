...
temprecΔ_ins_government_users public.government_users%ROWTYPE;
  BEGIN
    RAISE LOG 'dejima_bank_delta_action() called';
    IF NOT EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'dejima_bank_delta_action_flag') THEN
...
AS $$
  ...
  text_var3 text;
  BEGIN
    RAISE LOG 'dejima_bank_materialization() called';
    ...
    THEN
        ...
        AFTER INSERT OR UPDATE OR DELETE ON 
            __temp__Δ_ins_dejima_bank DEFERRABLE INITIALLY IMMEDIATE 
        ...
        CREATE CONSTRAINT TRIGGER __temp__dejima_bank_trigger_delta_action
        AFTER INSERT OR UPDATE OR DELETE ON 
            __temp__Δ_del_dejima_bank DEFERRABLE INITIALLY IMMEDIATE 
            FOR EACH ROW EXECUTE PROCEDURE public.dejima_bank_delta_action();
    END IF;
    RETURN NULL;
  EXCEPTION
...
AS $$
  ...
  text_var3 text;
  BEGIN
    RAISE LOG 'dejima_bank_update() called';
